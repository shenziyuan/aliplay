...
class TrackUIModel {
  ...
  TrackUIModel({this.selValue, required this.isOpen, required this.title,required this.children});
}
...
class SubTrackUIModel {
  String title;
  var value;
  SubTrackUIModel({required this.title, this.value});
}

class TrackFragmentState extends State<TrackFragment> {
  late List<TrackUIModel> _list;
    ...
    widget.fAliplayer.getMediaInfo().then((value) {
      ...
      if (info.tracks!.length > 0) {
        ...
        info.tracks!.forEach((element) {
          SubTrackUIModel model = SubTrackUIModel(title: '');
